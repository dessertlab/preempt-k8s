...
while getopts "f:t:i:e:m:p:q:o:d:n:s:r:b:u:c:h" opt; do
    case $opt in
        ...
        h) 
            ...
            echo "  -b <base-scale>                 Base scale for the tested service (default: 0)"
...
if [ "$BASE_SCALE" -eq 0 ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Waiting for service to scale-down to $BASE_SCALE instance(s)..."

    if [ "$FORCE_CLEANUP" == "true" ]; then
        TIMEOUT_SECONDS=90
        ELAPSED=0
        while [ $(kubectl get pods --no-headers | { grep "^rnn-serving-python" || true; } | { grep "Terminating" || true; } | wc -l) -ne 1 ]; do
        ...
        TERMINATING_POD=$(kubectl get pods --no-headers | { grep "^rnn-serving-python" || true; } | { grep "Terminating" || true; } | awk '{print $1}')
        kubectl delete pod "$TERMINATING_POD" --grace-period=0 --force &>/dev/null || true
    fi
