...
func main() {
	...
	//ctx = filteredinformerfactory.WithSelectors(ctx, serving.RevisionUID) //We don't look for pods by Revision UID
	ctx = filteredinformerfactory.WithSelectors(ctx, serving.RevisionUID, serving.RevisionLabelKey) //We look for pods by Revision Name
