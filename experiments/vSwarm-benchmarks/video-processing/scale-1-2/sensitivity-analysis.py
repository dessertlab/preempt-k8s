...
def save_comparative_boxplot(data_km_15, data_pk8s_15, data_km_30, data_pk8s_30, 
    ...
    # """
    """
    Create a sensitivity analysis boxplot with 4 boxes (2 parameter values x 2 controllers).
    Shows how metrics vary with parameter values (15, 30) for both controllers.
    """
    ...
    positions = [1, 2, 4, 5]  # Grouped positions
    ...
    colors = [color_km, color_pk8s, color_km, color_pk8s]
    ...
    # X-axis: show interfering-load parameter values as group ticks (15,30)
    # group_positions = [1.5, 4.5, 7.5]
    # group_labels = ['15 Int', '30 Int', '45 Int']
    group_positions = [1.5, 4.5]
    group_labels = ['15 Stressload', '30 Stressload']
...
def save_comparative_cdf_plot(data_km_15, data_pk8s_15, data_km_30, data_pk8s_30,
    ...
    fig, axes = plt.subplots(2, 1, figsize=(24, 10), sharex=True, constrained_layout=True)
...
def process_experiment_data(root_path, num_services, controller_name):
    ...
    for iter_idx, audit_file in enumerate(audit_files):
        ...
        for i in range(num_services):
            service_id = f"video-processing-python-{i+1}"
            try:
